...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # mark the cell as safe cell, update any sequence that contains the cell as well
        self.mark_safe(cell)
        ##2
        ...
        for cell in neighbours:
            if cell in self.mines:
                count_cpy -= 1
            if cell not in self.mines | self.safes:
                #uncertain
                cells.add(cell)
        
        sent = Sentence(cells, count_cpy)
    ...
    def infer(self):
        # infer new information about the board based on current knowledge
        for s1 in self.knowledge:
            for s2 in self.knowledge:
                #subset
                if s1.cells.issubset(s2.cells):
                    extras = s2.cells - s1.cells
                    count_ = s2.count - s1.count
                    sent = Sentence(extras, count_)
