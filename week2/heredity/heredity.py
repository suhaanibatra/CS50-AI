...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    zero_gene = people.keys() - (one_gene | two_genes) #people not in one_gene and two_genes are in zero_gene

    for person in zero_gene:
        # probability that each parent will not contribute gene => no gene for child
    ...
    for person in one_gene:
        ...
        if people[person]["mother"] is None:
            ...
            if mother in zero_gene and father in zero_gene:
                probability *= PROBS["mutation"] * (1 - PROBS["mutation"]) + (1 - PROBS["mutation"]) * PROBS["mutation"]
            ...
            if mother in one_gene and father in zero_gene:
                probability *= 0.5 * (1 - PROBS["mutation"]) + PROBS["mutation"] * 0.5
